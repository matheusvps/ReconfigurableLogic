...
unsigned int dm9000a_reset(alt_u8 *mac_address)
{
  unsigned int  i;
...
unsigned int TransmitPacket(alt_u8 *data_ptr, unsigned int tx_len)
{
  unsigned int i;
...
{  
  ...
  ifp->n_mib->ifPhysAddress = (alt_u8*)dm9ka->mac_addr;
  ifp->n_mib->ifDescr = (alt_u8*)"DM9000A series ethernet";
  ...
  while(rx_rdy == DM9000_PKT_READY)
  {
    ...
    if( !(rx_sts & 0xBF00) && (rx_len < MAX_PACKET_SIZE) )
    {
      ...
      { /* packet allocation succeeded */
        alt_u8* data_ptr = pkt->nb_buff + ETHHDR_BIAS;
